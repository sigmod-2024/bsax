...
class iSAX2Tree {
private:
    void DFS(const SAX *search_sax, void *node, bool is_leaf, std::vector<LeafKey> &leaf_ans, CARD *card_now) const;
...
public:
    iSAX2Tree(const char* _filename): filename(_filename) {}
    void Build();
    void BuildFromSAX(const char* sax_filename);
    void BuildTree();
    void Insert(const SAX* insert_sax, u_int64_t p);
